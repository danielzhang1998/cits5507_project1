...
double *main_quick_and_omp_quick(double *array, size_t array_length, struct timeval start, struct timeval middle, struct timeval end)
{
    ...
    printf("quick:\n");
    double no_omp = common_quick::print_time_distance(start, middle);
    double *omp_quick_result = start_omp_quick(array, array_length);

    gettimeofday(&end, NULL);
    printf("quick using omp:\n");
    double has_omp = common_quick::print_time_distance(middle, end);
    common_quick::compare_result(array_quick, array_quick, omp_quick_result, array_length);
    common_quick::print_ratio(no_omp, has_omp);
...
double *start_omp_quick(double *array, size_t array_length)
{
    double *array_omp_quick = common_quick::deep_copy(array, 0, array_length);
